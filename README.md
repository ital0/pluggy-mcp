...
| `PLUGGY_MCP_RATELIMIT_PER_MIN` | no | `30` | Per-tool budget over a 60-second sliding window. | Non-positive or non-numeric falls back to the default and logs `config_invalid`. Budget is **per tool**, **in-memory only**, and **resets on process restart**. The same default applies to every tool — including high-PII `getRawAccountDetails`, `getIdentity`, and `getIdentityByItem`; configure tighter limits if needed. |
| `PLUGGY_MCP_RATELIMIT_PER_DAY` | no | `200` | Per-tool budget over a 24-hour sliding window. | Same fallback, per-tool, in-memory, and restart-reset behavior as above. |
