...
/**
 * Sentinel `code` value carried on Errors thrown by `ensureOutputShape`.
 * The classifier recognises it and maps to the `INTERNAL` ErrorCode so the
 * audit line records `outcome=error` with a stable, branchable code.
 */
export const OUTPUT_SCHEMA_MISMATCH_CODE = 'OUTPUT_SCHEMA_MISMATCH';
...
export function ensureOutputShape<T extends ZodTypeAny>(
  ...
  if (!result.success) {
    ...
    (err as Error & { code?: string }).code = OUTPUT_SCHEMA_MISMATCH_CODE;
    throw err;
  }
}
